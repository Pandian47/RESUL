...
import { ListAqusitionSekelton, PieChartSkeleton } from 'Components/Skeleton/Skeleton';
import {
    FormatAudienceConversionChartData,
    FormatByInterestPieChartData,
    FormatKnownRetentionData,
    FormatPieChartData,
    Format_active_visitor_traffic_ChartData,
    FormatgetByHoursMobile,
    FormatgetUserBehaviorByDays,
    WEB_LIVE_DASHBOARD_DATA,
    dd_retention,
    dd_top_page_views,
    findByDaysMaxStateAndPercentage,
    findMaxEngagementTimeAndPercentage,
    getFullDayName,
    getProductAnalyticsFunc,
    hasChartData,
    hasProductInterestWebData,
    hasProgressiveProfileWebData,
    isAllValuesZero,
} from './constants';
import {
...
import {
    ...
    TOP_EVENT_SUMMARY,
    TOP_PAGE_VIEWS,
    TRAFFIC_BREAKDOWN_WEB_VISITORS,
    UNIQUE_WEB_VISITORS,
    VISITORS_ENGAGE,
    VISITOR_STATUS,
    WAU,
...
const LiveVisitorCount = memo(({ isWebAppId }) => {
    ...
    useEffect(() => {
        ...
        const getWebUserCount = (usersMap, domainName) => {
            const users = new Map();

            for (const [, value] of usersMap) {
        ...
        const startListener = (user, domainName) => {
            if (!user?.socketId) return;
            webLiveUsers.set(user.socketId, user);
            getWebUserCount(webLiveUsers, domainName);
        };
        ...
        const connectSocket = () => {
            ...
            let domainName = isWebAppId?.id;
            if (domainName?.includes('www.')) {
                domainName = domainName.replace('www.', '');
            }
            if (!domainName) return;

            socket = window.io.connect(WEBLIVESOCKETURL, {
            ...
            socket.on(domainName, (msg) => {
                startListener(msg, domainName);
            });
    ...
    return <h3 className="user-count">{formatNumber(displayCount)}</h3>;
});

LiveVisitorCount.displayName = 'LiveVisitorCount';

const WebLiveAudienceDashboard = () => {
    // Selectors
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const { isWebAppData = [], isWebAppId = {} } = useSelector((state) => state?.globalstate ?? {});
    ...
    const webDomainsAPI = useApiLoader({ actionCreator: getWebDomains });
    const audienceCountAPI = useApiLoader({ actionCreator: getAudienceCountMobile, autoFetch: false });
    const webAppDashboardAPI = useApiLoader({ actionCreator: getWebAppDasdhboard, autoFetch: false });
    ...
    const webPathAnalyserAPI = useApiLoader({ actionCreator: getPathAnalyser, autoFetch: false });
    ...
    const [pathAnalyserDData, setPathAnalyserDData] = useState('3 Levels');
    ...
    const [audienceCountData, setaudienceCountData] = useState({ totalCount: 0, liveCount: 0 });
    const [isLoading, setIsLoading] = useState(true);
    const [maxHours, setMaxHours] = useState({ maxHoursPercentage: 0, maxHoursTimeRange: 0 });
    const [byDaysMaxState, setByDaysMaxState] = useState({ maxState: null, maxPercentage: 0 });
        ...
        (list, dataSource) => {
            ...
            return list
                .map((item) => {
                    ...
                    return found?.attributeName != null ? found.attributeName : value;
                })
                .filter(Boolean)
                .join('-');
        },
    ...
    const handlegetPathAnalyser = useCallback(
        async (industriesListOverride, sankeyDataOverride) => {
            ...
            try {
                ...
                const res = await webPathAnalyserAPI.refetch(payload);
                if (!isMounted.current) return;
                if (res?.status) {
                    const { pathAnalyzerList, pathAnalyserfulldata: pathAnalyserFullData } = res ?? {};
        ...
        [isWebAppId, clientId, departmentId, userId, webPathAnalyserAPI, getValues, generatePathAnalyzerString],
    );
    ...
    }, [clientId, departmentId, userId, pathAnalyserDDLAPI, setValue, isWebAppId?.id, handlegetPathAnalyser]);

    const handleGetCount = useCallback(async () => {
        if (!isWebAppId?.id) return;
        ...
        try {
            ...
            setaudienceCountData((prev) => ({
                ...prev,
                totalCount: res?.status ? (res?.webpushcount ?? 0) : 0,
            }));
        } catch (err) {
            if (isRequestAborted(err)) return;
            if (!isMounted.current) return;
    ...
    }, [isWebAppId, clientId, departmentId, userId, audienceCountAPI]);

    const handleGetWebAppData = useCallback(async () => {
        if (!isWebAppId?.id) return;
        ...
        try {
            ...
            const res = await webAppDashboardAPI.refetch(payload);
            if (!isMounted.current) return;
            if (res?.status) {
                setWebData(res?.data ?? {});
            } else {
    ...
    }, [isWebAppId, clientId, departmentId, userId, webAppDashboardAPI]);
    ...
    const handleGetLiveCountData = useCallback(async () => {
        ...
        try {
            ...
            setaudienceCountData((prev) => ({
                ...prev,
                liveCount: res?.status ? (res?.weblivecount ?? 0) : 0,
            }));
        } catch (err) {
            if (isRequestAborted(err)) return;
            if (!isMounted.current) return;
    ...
    }, [isWebAppId, clientId, departmentId, userId, liveCountAPI]);

    const getWebDomainsData = useCallback(async () => {
        const payload = {
            clientId,
            departmentId,
            userId,
        ...
        try {
            const res = await webDomainsAPI.refetch({ payload, isPageHeader: true });
            if (!isMounted.current) return;
            if (res?.status) {
    ...
    }, [clientId, departmentId, userId, dispatch, webDomainsAPI]);
    ...
    useEffect(() => {
        if (!isWebAppId?.id) {
            if(!isSessionStale && !webDomainsAPI?.isFetching){
                setIsLoading(false)
            }
            return;
        }
        ...
        setPathAnalyserfulldata([]);
        setSankeyDropdownData([]);
        setPathAnalyserDData(`${initialMaxLevel} Levels`);
        setIsChartRendering(false);
    ...
    useEffect(() => {
        if (!isEmpty(webData)) {
            setRetentionData(webData?.retentionknownWeb ?? []);
        }
        let data = [];
        try {
            const productEntry = webData?.productAnalyticksWeb?.[0];
            if (productEntry) {
                const parsed = JSON.parse(Object.values(productEntry)[0])?.[0]?.Product_Analytics;
                data = parsed ?? [];
            }
        } catch {
            data = [];
        ...
        const audienceType2 = audienceType?.length ? Object.values(audienceType[0] ?? {}) : [];
        const sumData = audienceType2?.length ? audienceType2.reduce((x, y) => Number(x) + Number(y), initialValue) : 0;
    ...
    const checkAllChildrenZero = (children) => {
        return children?.every(
            (child) => child?.size === 0 || (child?.children && checkAllChildrenZero(child?.children)),
        );
    };
    ...
    const showTabSkeleton = isSessionStale || webDomainsAPI?.isFetching || isLoading;
    console.log('hasWebDomains: ', hasWebDomains);
    console.log('isLoading: ', isLoading);
    console.log('isSessionStale: ', isSessionStale);
    console.log('showTabSkeleton: ', showTabSkeleton);
    ...
    if (!hasWebDomains && !webDomainsAPI?.isFetching && !isSessionStale) {
        return (
            <>
                <div className="mt10">
                    <RSSkeletonTable
            ...
            <Row>
                ...
                <Col md={8}>
                    <div className="portlet-container portlet-sm">
                        ...
                        <div className="portlet-body">
                            {isAllValuesZero(webData?.dayWiseCounts) ? (
                                <DbSkPortletInlineSkeleton>
                                    <ListAqusitionSekelton
                                        isChartSkeleton
                                        isCustom
                                        isError={!isLoading && isAllValuesZero(webData?.dayWiseCounts)}
            ...
            <Row>
                <Col md={4}>
                    <div className="portlet-container keymetrics-portlet keymetrics-sm live-dashboard">
                        <h4>
                            {KEY_METRICS}
                            {hasChartData(webData?.screenWiseCountList) && (
                                <small className="d-inline-block pl5">({LAST_28_DAYS})</small>
                            )}
                        </h4>
                        {isAllValuesZero(webData?.screenWiseCountList) ? (
                            <DbSkPortletInlineSkeleton>
                ...
                <Col md={8}>
                    <div className="justify-content-inherit keymetrics-portlet keymetrics-sm live-dashboard portlet-container">
                        <h4>
                            {ACTIVE_USERS}
                            {hasChartData(webData?.userDevices) && (
                                <small className="d-inline-block pl5">({LAST_28_DAYS})</small>
                            )}
                        </h4>
                        {isAllValuesZero(webData?.userDevices) ? (
                            <DbSkPortletInlineSkeleton>
                                    ...
                                    {levels?.length > 0 ? (
                                        <>
                                            ...
                                            <DbSkPathAnalyserChartSlot
                                                ...
                                                isError={
                                                    !isPathAnalyserLoading && !isChartRendering && !pathAnalyser?.length
                                                }
                                            >
                                                <RSHighchartsContainer
                                                    type="sankey"
                                                    ...
                                                    options={sankeyChartOptions({
                                                        series:
                                                            isPathAnalyserLoading || isChartRendering
                                                                ? []
                                                                : pathAnalyser,
                    ...
                    <div className="portlet-container portlet-md">
                        ...
                        <div className="portlet-body">
                            {isAllValuesZero(webData?.knownUserWeb) ? (
                                ...
                                <RSHighchartsContainer
                                    type="pie"
                                    options={pieChartOptions(FormatAudienceConversionChartData(webData?.knownUserWeb))}
                                    smallText={LAST_28_DAYS}
                                />
                            )}
                        </div>
            ...
            <Row>
                <Col md={6}>
                    <DashBoardCard
                        ...
                        isError={!isLoading && isAllValuesZero(webData?.userScreens)}
                    />
                </Col>
                <Col md={6}>
                    <DashBoardCard
                        ...
                        smallText={LAST_28_DAYS}
                        type={'web'}
                        isError={!isLoading && isAllValuesZero(webData?.eventCountWebList)}
                    />
                </Col>
            </Row>
            ...
            <Row>
                        ...
                        tabData={[
                            {
                                ...
                                component: () => (
                                    <>
                                        {isAllValuesZero(webData?.webWiseCountCustList) ? (
                                            <DbSkPortletInlineSkeleton>
                                                <DbSkPieChartSkeleton isError={!isLoading} />
                                            </DbSkPortletInlineSkeleton>
                                        ) : (
                            ...
                            },
                            {
                                ...
                                component: () => (
                                    <>
                                        {isAllValuesZero(webData?.webVersionList) ? (
                                            <DbSkPortletInlineSkeleton>
                                                <DbSkPieChartSkeleton isError={!isLoading} />
                                            </DbSkPortletInlineSkeleton>
                                        ) : (
                                            <RSHighchartsContainer
                                                ...
                                                key={'by_browser'}
                                                smallText={LAST_28_DAYS}
                                                options={pieChartOptions(FormatPieChartData(webData?.webVersionList))}
                                            />
                                        )}
                                    </>
                                ),
                ...
                <Col md={6} className="usage_behav">
                    <RSChartTabbar
                        ...
                        defaultClass={`tabTransparent pt0`}
                        activeClass={`active`}
                        footer={hasChartData(webData?.byDaysCountWeb) && hasChartData(webData?.hourWiseCounts)}
                        containerClass={
                            hasChartData(webData?.byDaysCountWeb) && hasChartData(webData?.hourWiseCounts)
                                ? 'pfooter'
                                : ''
                        }
                        tabData={[
                            {
                                ...
                                component: () => (
                                    <>
                                        {isAllValuesZero(webData?.byDaysCountWeb) ? (
                                            <DbSkPortletInlineSkeleton>
                                            ...
                                            <>
                                                <RSHighchartsContainer
                                                    ...
                                                    options={columnChartOptions(
                                                        FormatgetUserBehaviorByDays(webData?.byDaysCountWeb, true),
                                                    )}
                                                    smallText={LAST_28_DAYS}
                            ...
                            },
                            {
                                ...
                                component: () => (
                                    <>
                                        {isAllValuesZero(webData?.hourWiseCounts) ? (
                                            <DbSkPortletInlineSkeleton>
