...
// import { getGalleryDetails } from 'Reducers/analytics/communicationAnalytics/request';
import { updateAnalyticsDetail } from 'Reducers/analytics/communicationAnalytics/reducer';
import { buildDetailAnalyticsNavState, buildAnalyticsReportReturnPath } from '../../../../DetailedAnalytics/constants';
...
const CommunicationAnalysisTable = ({ list, stateVal, retarget, index }) => {
    ...
    const splitScheduleColumns = useMemo(() => {
        const baseColumns = [
            ...
            { field: 'Days', title: 'Date', width: 140 }
        ];
        ...
        if (channelId === 1) { // Email
            specificColumns = [
                ...
                { field: 'uniqueOpens', title: 'Unique opens', width: 150, className: 'text-right', headerClassName: 'text-right' },
    ...
    const locationState = useQueryParams('/analytics/analytics-report');
    ...
    const splitScheduleData = useMemo(() => {
        ...
        return rawData.map((item, i) => {
            ...
            return {
                ...
                Days: getUserCurrentFormat(processedItem.scheduleDateTime)?.dateFormat
            };
        });
    ...
    const buildNavigationState = () => {
        ...
        return buildDetailAnalyticsNavState({
            channelName: blastName,
            channelId: channelId === 7 || channelId === 10 ? channelId : stateVal?.channelId === 10 ? stateVal.channelId : channelId,
            subChannelId: list?.subChannelId ?? list?.subchannelId,
            ...
            splitName: isWinnerSplit
                ...
                : list?.splitType
                  ...
                  : '',
            splitType: list?.splitType,
            subSegmentLevel: summarySubSegmentDetail?.subSegmentLevel ?? locationState?.subSegmentLevel,
        ...
        <div className="theme-radius-bottom">
            {isTableView ? (
                <div className="w-100 pt15">
                    ...
                    <div className="analysis-table-wrapper pb15">
                        <KendoGrid
                            data={splitScheduleData}
                            column={splitScheduleColumns}
                            pageable={true}
                            settings={{
                                                    total: splitScheduleData?.length,
                                                }}
                ...
                <Row className={isTableView ? '': 'pt15'}>
            ...
            <RSModal
                ...
                body={
                    <Fragment>
                        <div className={` ${[8,14]?.includes(channelId) ? '' : 'modal-img-box'}  ${[2, 21, 41]?.includes(channelId) ? '' : 'css-scrollbar'} analytics-modal-preview analytics-modal-preview`}>
                            {channelId === 1 ? (
                                <EmailListPreview
                                    data={{
                                        content: previewData.content,
                                ...
                                GetpopoverContentPlanner({ channelId, content: showModal.modalInfo, senderName: previewData.senderName, previewImage: previewData.previewImage, scheduleDate: previewData.scheduleDateTime, carouselJSON: previewData.carouselJSON, isCarousel: previewData.isCarousel, header: previewData.header, footer: previewData.footer, imagePath: previewData.previewImage, contentJson: previewData?.contentJson, className: 'rs-listing-messaging-preview float-none mx-auto', previewSource: PREVIEW_SOURCE.CSR })
                            )}
