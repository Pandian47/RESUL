...
export function mergeApiCountWithFactModelTotal(apiList, factModel) {
    ...
    return apiList.map((item) => {
        const apiCount = Number(item?.count) || Number(item.data?.[0]?.firstTouch) || 0;
...
/** Same currency symbol resolution as PerformanceAndInsights / RoiChart tooltip (user profile + master list). */
...
/**
 * Tooltip payload for GetSingleandMultiTouch `percentage[]` rows.
 * @param {string} [touchLabel] — 'First touch' | 'Last touch' (matches TOUCH_DROPDOWN_DATA); only that block is shown.
 */
    ...
    if (Array.isArray(apiData)) {
        return (apiData || []).reduce(
            (sum, item) =>
                sum +
                (Number(item?.count) ||
                    Number(item?.first_touch_count) ||
                    Number(item?.firstTouchCount) ||
                    0),
            0,
        );
    }
...
function mapArrayResponseToRoiSingleTouch(apiData) {
    ...
    return apiData
        .map((item) => {
            ...
            if (!config) return null;
            const hasTouchFields =
                item?.first_touch_pct != null ||
                item?.first_touch_count != null ||
                item?.last_touch_pct != null ||
                item?.last_touch_count != null;

            const count = hasTouchFields ? Number(item?.first_touch_count) || 0 : Number(item?.count) || 0;
            const roiCount = hasTouchFields ? Number(item?.last_touch_count) || 0 : Number(item?.roiCount) || 0;
            const roiAmount = Number(item?.roiAmount ?? item?.amount ?? 0);

            const firstTouchVal = hasTouchFields ? Number(item?.first_touch_pct) || 0 : count;
            const lastTouchVal = hasTouchFields ? Number(item?.last_touch_pct) || 0 : roiCount;
            return {
                name: config.name,
                icon: config.icon,
                color: config.color,
                ...
                roiCount,
                roiAmount,
                data: [{ firstTouch: firstTouchVal, lastTouch: lastTouchVal }],
            };
        })
        .filter(Boolean);
}
