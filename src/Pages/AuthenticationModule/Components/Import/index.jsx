...
const Import = ({ fieldName = '', isSplit = false, showBrowerText = false, isNotification = false, channelId }) => {
    ...
    const setIframeHeight = (iframe, body) => {
        ...
        if (!iframe || !body || !doc || !html) return;
    ...
    const importCampaignFromUrl = useCallback(
        async (url) => {
            ...
            const response = await importUrlLoader.refetch({
    ...
    return (
        <>
                    <div
                        className={`rs-import-block ${importType === 'url' ? 'import-url-blank' : 'import-url-selected'
                            }`}
                    >
                            ...
                            <div className="rs-import-refresh">
                                {(edmContent?.length > 0 ||
                                    ...
                                    <RSTooltip text="Reset" position="top">
                                        <i
                                            id="rs_data_refresh"
                                            className={`${restart_medium} icon-md color-primary-blue cp ${edmContent?.length > 0 ||
                                                    zipFileNameText?.[0]?.name?.split('.')?.pop() === 'zip' ||
                                ...
                                {importType === 'url' && (
                                    <Fragment>
                                        ...
                                        <Col sm={2} className="pl0 rsiuw-3 mt-12">
                                            <RSPrimaryButton
                                                className={`pr20 ${!importName || checkImportUrlError || importUrlLoader.isFetching ? 'click-off' : ''
                                                    }`}
                                                id="rs_Import_Go"
                                ...
                                >
                                    <div className="rs-import-with-icon or-sep">
                                        <div className="import-zip-file-tab text-center position-relative top4">
                                            ...
                                            <Fragment>
                                                <input
                                                    disabled={zipUploadLoader.isLoading}
                                                    onClick={() => {
                                                    ...
                                                    {...register(zipFileName, {
                                                                        ...
                                                                        reader.onload = async () => {
                                                                            ...
                                                                            if (isNotification) {
                                                                                ...
                                                                                if (status) {
                                                                                    ...
                                                                                    if (message.startsWith('AMP Validation') || message.includes('AMP Validation')) {
                                                        ...
                                                        validate: async (value) => {
                                                            ...
                                                            else if (!edmContent && uploadAttemptCount.current > 1) return 'Select zip file';
                                                            else {
                                                                if (edmFileWeight > 5242880)
                                                                    return 'File size too large';
                                                                return true;
                                            ...
                                            <div className="import-zip-message position-relative">
                                                {zipUploadLoader.isLoading && (
                                                    <span
                                                        ...
                                                        id="rs_import_zip_upload_loader"
                                                    >
