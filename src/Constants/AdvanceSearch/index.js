...
export function buildCommunicationAnalyticsAdvanceFilterConfig({ attributeList = [], productList = [] }) {
    return [
        {
            key: 'sort_type',
            label: 'Sort type',
        ...
        },
        {
            key: 'delivery_type',
            label: 'Delivery type',
            data: [...ANALYTICS_DELIVERY_TYPE_FILTER_DATA],
            isObject: true,
            fieldKey: 'name',
            isMulti: true,
        },
        {
            ...
            data: [
                { attributename: 'All', campaignAttributeId: '0', isAll: true },
                ...attributeList,
            ],
            isObject: true,
        ...
        },
        {
            key: 'product_type',
            label: 'Product type',
            data: [{ categoryname: 'All', categoryId: 0, isAll: true }, ...productList],
            isObject: true,
...
export function firstNonAllFilterItem(items) {
    if (!Array.isArray(items) || !items.length) return null;
    const filtered = items.filter((x) => !x?.isAll);
