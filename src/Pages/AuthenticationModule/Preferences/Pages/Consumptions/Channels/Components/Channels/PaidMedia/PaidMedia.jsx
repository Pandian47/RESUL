...
const ConsumptionPaidMedia = () => {
    ...
    const [custColumns, setCustColumns] = useState(['dd', 'dd1', 'dd2']); //consumptionChannelList?.paidMediaChannelConsumptionList?.
    ...
    const handleConsumptionChannelDetails = async (params) => {
        let { status, data } = await dispatch(getConsumptionChannelDetails(params));
            ...
            const tableName =
                Array.isArray(data?.paidMediaChannelConsumptionList?.[0]?.custom) &&
                data.paidMediaChannelConsumptionList[0].custom?.length > 0
                    ? _map(data.paidMediaChannelConsumptionList[0].custom, (res, key) => {
    ...
    useEffect(() => {
        const tableName =
            Array.isArray(consumptionChannelList?.paidMediaChannelConsumptionList?.[0]?.custom) &&
            consumptionChannelList.paidMediaChannelConsumptionList[0].custom?.length > 0
                ? _map(consumptionChannelList.paidMediaChannelConsumptionList[0].custom, (res, key) => {
                      return { field: res, title: res, width: 250 };
                  })
                : [];
        setCustColumns(tableName);
        const data = consumptionChannelList?.paidMediaChannelConsumptionList?.map((item) => {
    ...
    const handleChannelName = (subChannelId) => {
        ...
        return paidMediaChannelName[subChannelId] || '';
    };
        ...
        <Fragment>
            {/* {consumptionChannelList?.length > 0 || consumptionChannelList ? ( */}
                <>
                                    ...
                                    <RSTooltip text={'Download CSV'}>
                                        <i
                                            onClick={() => {
                                                ...
                                                downloadCSV(
                                                    consumptionChannelList?.paidMediaChannelConsumptionList,
                    ...
                    {/* {consumptionChannelList?.paidMediaChannelConsumptionList !== null ? ( */}
                        <div className="mb70">
                            <KendoGrid
                    ...
                    {/* ) : (
                        <HorizontalSkeleton
                            isError={
                                consumptionChannelList?.paidMediaChannelConsumptionList === null ||
                                consumptionChannelList?.paidMediaChannelConsumptionList?.length === 0
                            }
                        />
                    )} */}
                </>
...
export default ConsumptionPaidMedia;
