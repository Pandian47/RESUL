...
import { save_Retargetlist } from 'Reducers/analyticsTwins/communicationAnalytics/request';
...
const RetargetList = ({
    ...
    const handleFormSubmit = async (data) => {
        ...
        const payload = {
            segmentationName: data?.listName,
            listType: 10,
                ...
                body={
                    <form onSubmit={handleSubmit(handleFormSubmit)}>
                        {channelId === 1 && label === 'Unique opens' && (
                          ...
                          {showMsg.status ? (
                            <div
                                className={`alert mb27 ${showMsg.msg?.includes('not')  ? 'alert-danger' : 'alert-success'
                                    }`}
                            >
                                <i
                                    className={`position-relative mr10 p5 white ${showMsg.msg?.includes('not') 
                                            ? alert_medium
                                            : circle_tick_medium 
