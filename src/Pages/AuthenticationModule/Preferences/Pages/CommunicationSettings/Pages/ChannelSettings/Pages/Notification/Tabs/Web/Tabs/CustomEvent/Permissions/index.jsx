...
const WebAppPermission = () => {
    ...
    const getData = useCallback(async () => {
        ...
        if (status && data) {
            ...
            const leftAttributes = data.attributes?.additional?.available.filter(
                (item1) => !data.attributes?.additional?.assigned.some((item2) => item1.attributeId === item2.attributeId),
            ) || [];
    ...
    }, [clientId, userId, departmentId, location?.id, dispatch, setValue, resetSelection]);

    useEffect(() => {
        if (location?.id) getData();
    }, [location?.id, getData]);
    ...
    const handleSave = useCallback(async (data) => {
        ...
        const res = await dispatch(UpsertWebPushPermission(payload));

        if (res?.status) {
            navigateToSettings();
        }
    }, [clientId, userId, departmentId, location?.id, pageGrid, tempAttrAssign, tempEventsAssign, gridData, wepAppPermissionData, dispatch, navigateToSettings]);
                    ...
                    <form onSubmit={handleSubmit(handleSave)}>
                        {hasData ? (
                            <>
                                ...
                                <ActionButtons onCancel={navigateToSettings} />
                            </>
                        ) : (
                            <>
                                <div className="">
                                ...
                                <ActionButtons onCancel={navigateToSettings} showSave={false} />
                            </>
                        )}
                    </form>
                </FormProvider>
...
const ActionButtons = ({ onCancel, showSave = true }) => (
    <div className="buttons-holder pref-cs-buttons-outside">
        <RSSecondaryButton type="button" onClick={onCancel}>
            {placeholder.CANCEL}
        </RSSecondaryButton>
        {showSave && <RSPrimaryButton type="submit">{placeholder.SAVE}</RSPrimaryButton>}
    </div>
);
