...
const AnalyticsDetail = ({ dataIndex, dataItem, onCollapse }) => {
    ...
    return (
        <div className="rs-grid-detail-view">
            <table
                className={`grid-detail-content ${isSubsegment ? 'subsegment' : ''}${
            ...
            >
                ...
                <tbody>
                    {!isFailure && dataItem?.channels && dataItem?.channels.length > 0 && (
                        <Fragment>
  ...
  {!isFailure && (!dataItem?.channels || dataItem?.channels.length === 0) && (
                    <div className="p15">
                         <RSSkeletonTable text count={2} isCustombox/>
                    </div>
                )}            
