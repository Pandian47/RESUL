...
export const createCSSMSSettings = (payload, handleCancel, loading = false) => async (dispatch) =>
    dispatch(
        request.post({
            url: CREATE_SMS_SETTINGS,
            payload,
            loading,
              ok: ({ data }) => {
                const { status, message = 'No data available' } = data;
...
export const getCSSMSUpdateGet = (payload) => async (dispatch) => {
    return dispatch(
        request.post({
            url: GET_UPDATE_RECORD_DATA,
            payload,
            loading: false,
            ok: () => {},
            isFailureCheck: true,
...
// TODO: Replace with actual API - returns { status: true } when name exists, { status: false } when available
export const checkSMSFriendlyNameExists = ({ payload }) => async (dispatch) => {
    const res = await dispatch(
        request.post({
            url: CHECK_SMS_FRIENDLY_NAME_EXISTS,
...
export const extendRegulatoryQuietHours = (payload) => async (dispatch) => {
    return dispatch(
        request.post({
            url: EXTEND_QUIET_HOURS_REGULATORY,
            payload,
            loading: true,
            fail: () => {},
        }),
...
export const GetWebAppList = (payload) => async (dispatch) => {
    return dispatch(
        request.post({
            url: GET_WEBMOBILE_LIST,
            payload,
            loading: true,
