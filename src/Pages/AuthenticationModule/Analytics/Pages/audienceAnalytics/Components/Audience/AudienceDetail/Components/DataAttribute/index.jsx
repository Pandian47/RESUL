        ...
        return Object.keys(data[0]).map((key) => {
            ...
            if (isDateField) {
                column.cell = ({ dataItem }) => {
                    ...
                    const formattedDate = dateValue ? getUserCurrentFormat(dateValue)?.dateTimeFormat || dateValue : '';
    ...
    const accordtionFunc = (result) => {
        ...
        const column = [
            ...
            },
            {
                ...
                title: 'Value',
                cell: ({ dataItem }) => (
                    <td>{formatAttributeNumericValue(dataItem.value, dataItem.solrFieldName, dataItem.name)}</td>
                ),
            },
        ];
        if (result?.length > 0) {
                    ...
                    if (column?.solrFieldName?.toLowerCase().trim() === key?.toLowerCase()?.trim()) {
                        if (column.GridShowStatus === 1) {
                            ...
                            const formattedValue =
                                isDateField && value ? getUserCurrentFormat(value)?.dateTimeFormat || value : value;
                            const currentObj = {
