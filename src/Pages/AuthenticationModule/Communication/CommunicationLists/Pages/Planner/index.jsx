...
const CommunicationPlanner = () => {
    ...
    const plannerListAPI = useApiLoader({ actionCreator: campaignPlannerList });
    const plannerAttributesAPI = useApiLoader({ actionCreator: communicationAttributes });
    ...
    const attributesList = Array.isArray(attributesData) ? attributesData : [];
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (!isAgencyAccountAdmin) {
            plannerListAPI.refetch({ payload: tempParams });
        }
    }, [payload, departmentId, clientId]);

    useEffect(() => {
        const attributesPayload = { departmentId, clientId, userId };
        if (departmentName?.toLowerCase() === 'all' && licenseTypeId === '3') {
            return;
        }
        if (!isAgencyAccountAdmin) {
            plannerAttributesAPI.refetch(attributesPayload);
        }
