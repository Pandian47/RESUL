...
const convertTo24Hour = (hours, minutes, amPM) => {
    if (amPM == 'PM' && hours < 12) hours = parseInt(hours, 10) + 12;
    if (amPM == 'AM' && hours == 12) hours = parseInt(hours, 10) - 12;

    var sHours = hours.toString();
    var sMinutes = minutes.toString();
    if (hours < 1 && hours?.length == 1) sHours = '0' + sHours;
    if (minutes < 1 && minutes?.length == 1) sMinutes = '0' + sMinutes;
    if (minutes?.length == 0) sMinutes = '00';

    return sHours + ':' + sMinutes + ':00';
};
const dateAdd = (date, p_Interval, p_Number) => {
