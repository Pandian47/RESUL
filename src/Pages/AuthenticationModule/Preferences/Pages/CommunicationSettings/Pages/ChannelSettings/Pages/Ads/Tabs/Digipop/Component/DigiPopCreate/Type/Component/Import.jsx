...
const Import = ({ fieldName, type = 'image', isPreview = true, isShowRadioField = true, isRequired = true, size }) => {
    const {
        control,
        clearErrors,
        setError,
                    ...
                    <Col sm={12}>
                        <div className="rs-import-block import-url-blank digipop-import">
                            <div className="form-group mb0 ">
                                {(imagePreview || watchUploadUrl[fileType] === 'Upload') && (
                                ...
                                <Row
                                    ...
                                    } ${watchUploadUrl[fileType] === 'Upload' ? 'click-off' : ''}`}
                                >
                                    <Col
                                        sm={2}
                                    ...
                                    <Col sm={5} className={`rsiuw-2 ${Importwatch ? 'd-none' : ''}`}>
                                        <>
                                            {type === 'image' && watchUploadUrl[fileType] !== 'Upload' ? (
                                                <RSInput
                                                    control={control}
                                                ...
                                                />
                                            ) : type === 'audio' && watchUploadUrl[fileType] !== 'Upload' ? (
                                                <RSInput
                                                    control={control}
                                                    name={getUniqueName(`${type}Url`)}
                                                    ...
                                                    rules={{
                                                        ...
                                                        validate: (val) => {
                                                            ...
                                                            if (!isValid && watchUploadUrl[imageType] !== 'Upload') {
                                                                return 'Enter valid URL';
                                                            }
                                                            return true;
                                                        },
                                                ...
                                                />
                                            ) : type === 'video' && watchUploadUrl[fileType] !== 'Upload' ? (
                                                <RSInput
                                                    control={control}
                                                    name={getUniqueName(`${type}Url`)}
                                    ...
                                    <Col sm={2} className={`pl0 rsiuw-3 ${Importwatch ? 'd-none' : ''}`}>
                                        <RSPrimaryButton
                                            ...
                                            className={`${imagePreview?.length ? '' : 'click-off'}`}
                                        >
                                            Go
                                        </RSPrimaryButton>
                                    </Col>
                                    <Col sm={Importwatch ? 10 : 3} className="pl0 pr0 rsiuw-4 digipopup-import">
                                        <div
                                        ...
                                        >
                                            <div className="import-zip-file-tab text-center position-relative top4">
                                                ...
                                                <span className="opacity-0  position-absolute">
                                                    {type === 'image' && watchUploadUrl[fileType] !== 'URL' ? (
                                                        <RSFileUpload
                                                            control={control}
                                                        ...
                                                        />
                                                    ) : type === 'video' && watchUploadUrl[fileType] !== 'URL' ? (
                                                        <RSFileUpload
                                                            control={control}
                                                            name={getUniqueName('video')}
                                                            accept={
                                                                '.mp4,.avi,.mov,.mkv,.wmv,.flv,.webm,.mpeg,.mpg,.mp3'
                                                            }
                                                            setError={setError}
                                                            clearErrors={clearErrors}
                                                            size={5000000}
