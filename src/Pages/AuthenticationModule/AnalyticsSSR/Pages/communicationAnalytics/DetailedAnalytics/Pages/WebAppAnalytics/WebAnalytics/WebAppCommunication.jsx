...
import { areachangeToBase64, changeToBase64, COMMUNICATION_PERFORMANCE, defaultValues, getDaywiseChartData, getLocationDetails, getPreviewData, handleChannelInfo, KEYWORD_RANKING_GRID_COLUMN_DATA, KEYWORD_RANKING_GRID_DATA, pieChartOption, TRAFFIC_BREAKDOWN_GRID_COLUMN_DATA, TRAFFIC_BREAKDOWN_GRID_DATA } from '../../../constants';
...
import { GetAnalyticsUser_Journey } from 'Reducers/analyticsSSR/details/request';
...
const WebAnalytics = ({ splitItem = 'Split A', type = 'post', channelName }) => {
    ...
    const {
     ...
     useEffect(()=>{
        ...
        if(overviewDetail?.device_metrics){
            const pie_structure = {
        ...
        if(overviewDetail?.browser_metrics){
            const pie_structure = {
    ...
    },[overviewDetail?.device_metrics , overviewDetail?.browser_metrics, ]);
    ...
    const processedFunnelSummary = useMemo(() => {
        const raw = userJourneyData?.step_summary || [];
        if (!Array.isArray(raw) || raw.length === 0) return [];
        return raw.map((it, i) => ({
            step_Name: it.label ?? it.step_Name ?? it.stepName ?? `Step ${i + 1}`,
            visits: it.visits ?? it.value ?? 0,
            drop_Offs: it.drop_offs ?? it.drop_Offs ?? it.dropOffs ?? 0,
            ...it,
        }));
    }, [userJourneyData]);
        ...
        <div className={`page-content ${true ? 'download-page-setup-detail' : ''}`}>
            ...
            {fromSplitHeader || Object.keys(overviewDetail || {})?.length > 0 ? (
                <div className="rs-csr-wrapper">
                                ...
                                tabData={[
                                    {
                                        ...
                                        component: () => {
                                            const chartData =
                                                reachPerformanceJson !== undefined &&
                                                    reachPerformanceJson !== null &&
                                                    reachPerformanceJson !== ''
                                                    ? changeToBase64(
                                                        reachPerformanceJson,
                                                        'area',
                    ...
                    {/* {WebAnalyticsData[type]?.[splitItem]?.gridData?.totalLinkClicks && ( */}
                    {/* <Row>
                        <Col md={12}>
                            <RSChartTabbar
                                dynamicTab={`mb0 mini`}
                                ...
                                autoHeight
                                tabData={['Direct', 'Organic Search', 'Referral', 'Social Media', 'Others'].map(
                                    (res, idx) => ({
                                        id: res,
                                        text: res,
                                        textClass: 'font-sm',
                                        component: () => (
                                            <div className="portlet-body" key={idx}>
                                                <KendoGrid
                                                    data={TRAFFIC_BREAKDOWN_GRID_DATA}
                                                    column={TRAFFIC_BREAKDOWN_GRID_COLUMN_DATA}
                                ...
                                tabData={[
                                    {
                                        ...
                                        />
                                                {/* <TabbarAreasPlineChart
                                                    ...
                                                    footerStatus={false}
                                                    overAllTextData={durationAreaData}
                                                    first24HoursData={{}}
                                                    content={true}
                                                /> */}
                                                </>
                    ...
                    <Col md={6} className="x-axis-labels-performance">
                        <RSChartTabbar
                            ...
                            endIcon
                            gridView={<KendoGrid {...VISITORS_FREQUENCY_GRID} />}
                            tabData={[
                                {
                                    component: () => {
                                        const chartData =
                    ...
                    <Col md={6} className="x-axis-labels-performance">
                        <RSChartTabbar
                            ...
                            endIcon
                            gridView={<KendoGrid {...VISITORS_REGENCY_GRID} />}
                            tabData={[
                                {
                                    component: () => {
                                        const chartData =
                            ...
                            <RSChartTabbar
                                ...
                                gridView={
                                    <KendoGrid
                                        ...
                                        { field: 'channel', title: 'Channel', width: 120 },
                                        { field: 'page_views', title: 'Page Views', width: 120 },
                                        { field: 'sessions', title: 'Sessions', width: 120 },
                                    ]}
                                        scrollable="scrollable"
                                        settings={{
                                            total: Array.isArray(device_metrics) ? device_metrics.length : 0,
                                        }}
                                        isScrollTop={false}
                                        pageable={true}
                                ...
                                tabData={[
                                    {
                                        ...
                                        component: () => {
                                            const chartData =
                                                Array.isArray(device_metrics) && device_metrics.length > 0
                                                    ? device_metrics.map((it) => ({ name: it.browser, y: Number(it.users) || 0 }))
                                                    : null;
                                            if (!chartData || chartData.length === 0 || !hasNonZeroPieChartSeriesData({ series: chartData })) {
                            ...
                            <RSChartTabbar
                                ...
                                gridView={
                                    <KendoGrid
                                        ...
                                        scrollable="scrollable"
                                        settings={{
                                            total: Array.isArray(browser_metrics) ? browser_metrics.length : 0,
                                        }}
                                        isScrollTop={false}
                                        pageable={true}
                                ...
                                tabData={[

                                    {
                                        ...
                                        component: () => {
                                            const chartData =
                                                Array.isArray(browser_metrics) && browser_metrics.length > 0
                                                    ? browser_metrics.map((it) => ({ name: it.browser, y: Number(it.users) || 0 }))
                                                    : null;
                                            if (!chartData || chartData.length === 0 || !hasNonZeroPieChartSeriesData({ series: chartData })) {
                                                return <PieChartSkeleton isError />;
                                            }
