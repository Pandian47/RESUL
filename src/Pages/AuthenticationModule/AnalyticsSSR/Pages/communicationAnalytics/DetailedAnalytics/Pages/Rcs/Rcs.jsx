...
import { CAMPAIGN_GRID_COLUMN_DATA_RCS, COMMUNICATION_PERFORMANCE, TOP_LINK_ACTIVITY, USER_ENGAGEMENT, changeToBase64, defaultValues, getPreviewData, handleChannelInfo, handleSegmentData, handleSplit, pieChartOption } from '../../constants';
...
const DetailAnalyticsRcs = ({ type, isDownloadUI }) => {
    ...
    const {
        ...
        // } = detailsList?.rcsDetailModel || defaultValues;
    ...
    const handlePreviewDetails = async (data) => {
        ...
        if(locationData?.iswinnerSplit && locationData?.iswinnerSplitType){
            getChannel_splitChannelLevelInfo = channelDetail?.splitChannelLevelInfo?.filter(    (item)=>
                item.splitType === locationData?.iswinnerSplitType
            );
        }
        ...
        let payload = {
            ...
            blastId: currentBlastId, //detailsList?.campaignGuid,
            ...
            {fromSplitHeader || Object.keys(overviewDetail)?.length > 0 ? (
                <div className="rs-csr-wrapper">
                    <SplitHeader
                        ...
                        splitViewCGTG={overviewDetail?.isRcsCgTg}
                        filterDropdown={
                            ...
                            <Row>
                                <Col md={6} className="x-axis-labels-performance">
                                    <TabbarAreasPlineChart
                                        ...
                                        footerStatus={
                                            !!enagegementPerformanceJson && enagegementPerformanceJson?.length !== 0 && hasNonZeroEngagementData(enagegementPerformanceJson)
                                                ? true
                                                : false
                                        }
                                        overAllTextData={
                            ...
                            <Row>
                                ...
                                <Col md={6}>
                                    <PieChart
                                        ...
                                        footerText={`of RCS have been ${
                                            customStatusContent?.length > 0 &&
                                ...
                                <Col md={6}>
                                    <PieChart
                                        ...
                                        footerText={`of RCS have been clicked from ${topOs}`}
                                    />
                                </Col>
                            </Row>
                            <Row>
                             ...
                             {!(channelDetail?.campaignType === 'T' || isHybrid || channelDetail?.isMDCTrigger || (filterDetails?.audienceChanged && ['3', '5'].includes(String(filterDetails?.selectedListType)))) && (
                                <Row>
                                    <DetailListDataTable
                                        columnListData={CAMPAIGN_GRID_COLUMN_DATA_RCS()}
                                        // campaignId={analyticsDetatils?.campaignId}
...
export default DetailAnalyticsRcs; 