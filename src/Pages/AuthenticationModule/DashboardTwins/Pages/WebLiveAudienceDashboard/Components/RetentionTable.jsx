...
const RetentionTable = ({ tableHeader, tableBody, striped }) => {
    ...
    return (
        ...
        //                 <th className="font-bold" key={index}>{header}</th>
        //             ))}
        //         </tr>
        //     </thead>
        //     <tbody>
                ...
                <div className="portlet-box-theme border">
                    <div className="tabs-content rs-table-wrapper shadow-none border-0">
                        <Table className="m0">
                            ...
                            {tableBody.map((item, index) => (
                                    <tr key={index}>
                                        <td className="p13">{item?.date}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.known)}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.day1)}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.day2)}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.day3)}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.day4)}</td>
                                        <td className="p13">{renderValueWithTooltip(item?.day5)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </Table>
