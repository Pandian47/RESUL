import { COMMUNICATION_RECOMMENDATIONS, COMMUNICATION_SUMMARY, GET_ATTRIBUTION_FOR_ROI, GET_BENCHMARK, GET_COMMUNICATION_PRE_BLAST, GET_COMMUNICATION_TRENDS, GET_CSR_PDFDOWNLOAD, GET_DEMOGRAPHICS, GET_GEOGRAPHY, GET_KNOWN_AND_UNKNOWN, GET_RETARGETLIST_STATUS, GET_SEGMENT_INDUSTRY, GET_SHOWLINK, GET_SNAP_DETAILS, GET_SNAP_NAME_LIST, GET_TOP_DEVICES, NEW_CONTACT_LIST, SAVE_SNAPSHOTS, SET_GOLDEN_CAMPAIGN, UNKNOWN_TO_KNOWN_CONVERSION } from 'Constants/EndPoints';
...
const getRadarColumnCategories = (series, categories) => {
    if (categories == null || categories === '') return [];
    if (Array.isArray(categories)) return categories;

    if (typeof categories === 'string') {
        const dateUtcMatches = categories.match(/Date\.UTC\([^)]+\)/g);
        if (dateUtcMatches?.length) {
            const categoryDates = dateUtcMatches.map((dateStr) => {
                try {
                    return eval(dateStr);
                } catch (_) {
                    return null;
            ...
            return _map(categoryDates, (date) =>
                date ? getUserCurrentFormat(date, { noConversion: true })?.dateFormat : date,
            );
        }

        const parsed = parseAnalyticsJsonArray(categories, null);
                    ...
                    if (status) {
                        ...
                        let tmpCategories =
                            key === 'line'
                                ...
                                : getRadarColumnCategories(series, categories);
...
export const getKnownToUnknownConversion =
    ({ payload }) =>
        async (dispatch) =>
            dispatch(
                request.post({
                    url: UNKNOWN_TO_KNOWN_CONVERSION,
                    payload,
                    // loading: true,
                    ok: ({ data }) => {
                        const { data: res, status } = data;
