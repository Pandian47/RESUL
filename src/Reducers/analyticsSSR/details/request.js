import { BIND_LINK_CSV_DOWNLOAD, CLICK_ACTIVITY_DETAILS, COMMUNICATION_STATUS, DETAIL_ADVANCE_SEARCH, DETAIL_ADVANCE_SEARCH_DATA, DETAIL_DOCKET, DETAIL_DOCKET_DOWNLOAD, DETAIL_DOWNLOAD_COMM_STATUS_ACTIVITY, DETAIL_EMAIL_HEATMAP_REPORT, DETAIL_GET_KYC_COUNT, DETAIL_REPORT, DETAIL_REPORT_CHANNELDETAILS, DETAIL_REPORT_CONTENTDETAILS, DETAIL_REPORT_DOWNLOAD, DETAIL_REPORT_OTPTOKEN, DETAIL_REPORT_OTPTOKEN_VALID, DETAIL_REPORT_OVERVIEWDETAILSSSE, DETAIL_REPORT_SEGMENTDETAILS, GET_ANALYTICSUSER_JOURNEY, GET_COMMUNICATION_PRE_BLAST, GET_DIGIPOP_REPORT, GET_FORM_ANALYTICS, GET_FORM_PROGRESSIVE_PROFILE } from 'Constants/EndPoints';
...
let sseRetryHandler = null;

export const closeDetailReportSSE = () => {
...
export const setDetailReportSseRetryHandler = (handler) => {
    sseRetryHandler = typeof handler === 'function' ? handler : null;
};

export const retryDetailReportOverview = () => async (dispatch) => {
    dispatch(setSseDisconnected(false));
    closeDetailReportSSE();

    if (sseRetryHandler) {
        await sseRetryHandler();
        return;
    }

    if (lastOverviewPayload) {
        return dispatch(getDetailReport_OverviewDetails({ payload: lastOverviewPayload }));
    }
};
...
const handleSseDisconnect = (dispatch, hasReceivedOverviewData) => {
    if (!hasReceivedOverviewData) {
        return;
    }
    dispatch(setSseDisconnected(true));
};
...
export const getDetailReport_OverviewDetails =
    ({ payload }) =>
        async (dispatch) => {
            ...
            const enhancedPayload = { ...payload, sessionId };
            lastOverviewPayload = { ...payload };

            // Open parallel Server-Sent Events (SSE) connection
            closeDetailReportSSE();

            try {
                const sseUrl = `https://wiz.resul.io/Sse/events?SessionId=${sessionId}&ClientId=${enhancedPayload.clientId}&TenantId=${enhancedPayload.clientId}&BuId=${enhancedPayload.departmentId}&CampaignId=${enhancedPayload.campaignId}&BlastId=${enhancedPayload.blastID || enhancedPayload.blastId || ''}`;
                const eventSource = new EventSource(sseUrl);
                activeEventSource = eventSource;
                let hasReceivedOverviewData = false;
                ...
                eventSource.onmessage = (event) => {
                    try {
                        ...
                        if (trimmed && (trimmed.startsWith('{') || trimmed.startsWith('['))) {
                            ...
                            const overviewData = res?.data !== undefined ? res.data : res;
                            if (overviewData && Object.keys(overviewData).length > 0) {
                                hasReceivedOverviewData = true;
                            }
                            dispatch(updateDetailsMainList({ field: 'overviewDetail', data: overviewData || {} }));
                        }
                    } catch (e) {
                        console.error('[SSE Parse Error] Failed to parse message:', e);
                    }
                };

                eventSource.onerror = () => {
                    const wasConnectedWithData = hasReceivedOverviewData;
                    eventSource.close();
                    if (activeEventSource === eventSource) {
                        activeEventSource = null;
                    }
                    handleSseDisconnect(dispatch, wasConnectedWithData);
                };
            } catch (err) {
                console.error('[SSE Setup Error] Failed to initialize SSE:', err);
            }

            return dispatch(
                request.post({
                    url: DETAIL_REPORT_OVERVIEWDETAILSSSE,
                    loading: false,
                    payload: enhancedPayload,
                    ok: (res) => {
                        // POST API serves to trigger the calculation/session; binding is handled purely by the SSE socket to avoid race condition overwrite.
                    },
                    fail: (err) => {
                    },
                    final: () => {
...
export const requestKeyPersonOTPAnalytics = (payload, setMessage, resend = false) => async (dispatch) => {
    return dispatch(
        request.post({
