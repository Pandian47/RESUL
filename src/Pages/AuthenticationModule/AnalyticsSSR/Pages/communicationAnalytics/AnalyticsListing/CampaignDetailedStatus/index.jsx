...
import { GetpopoverContent, getListingPreviewNoDataPopover, hasListingPreviewApiContent } from 'Utils/modules/preview';
...
const DetailedStatus = ({ content, dataItem, idx, splitIndex = null, hasGrouping = false }) => {
    ...
    const [preview, setPreview] = useState(GetpopoverContent(content?.channelId, initialProps));
    ...
    return (
        <Fragment>
                        ...
                        >
                            {isPreviewLoaded ? (
                                <RSPPophover
                                    position="left"
                                    {...preview}
                                    popover_overlay_class={`analyticslistpreview ${content?.channelId === 7 ? 'p0 border-0 border-r10' : ''}`}
                                    trigger="click"
                                >
                                    <span className="eye-icon-wrapper d-flex" ref={popoverTriggerRef}>
                                        <RSTooltip text="Preview" position="top" className={`${
                                content?.channelId === 6 || content?.channelId === 16 || content?.channelId === 10
                                    ? 'pe-none click-off'
                                    : ''
                            }`}>
                                            <i
            ...
            {popupStatus && (
                <SplitABScheduler
                    show={popupStatus}
                    handleClose={() => setPopupStatus(!popupStatus)}
