...
} from 'Reducers/analyticsSSR/communicationAnalytics/request';
...
import { mapAnalyticsQueryOptionsResponseToOptionsState } from 'Pages/AuthenticationModule/Analytics/Pages/communicationAnalytics/analyticsQueryOptionsMapper';
import {
...
import { coerceAnalyticsSummaryApiResult } from 'Pages/AuthenticationModule/Analytics/analyticsDefaults';
...
import analyticsListingInitialState from 'Reducers/analyticsSSR/communicationAnalytics/initialState';
...
const CommunicationAnalytics = () => {
    ...
    const { isLoading, isFailure, isGridLoading } = useSelector(
        (state) => state.analyticsListingSSRReducer ?? analyticsListingInitialState,
    );
    ...
    const expandChange = async ({ dataItem, dataIndex }) => {
        ...
        if (!dataItem?.expanded) {
            ...
            const expandResult = await dispatch(getAnalyticsInfoList(expandPayload));
            const status = expandResult?.status;
            const data =
            ...
            setAnalyticsListData((prevState) => {
                ...
                const itemIndex = dataInstance.findIndex(
                    (item) => item?.campaignID === itemId || item?.campaignGUID === itemId,
                );

                if (itemIndex >= 0) {
                    dataInstance[itemIndex] = {
                        ...dataInstance[itemIndex],
                        channels: Array.isArray(data?.campaignsListInfo) ? data.campaignsListInfo : [],
                        expanded: true,
                    };
                }
                return { 
            ...
            setAnalyticsListData((prevState) => {
                // Collapse everything when the expanded one is toggled off.
                const dataInstance = (prevState.analyticsList || []).map((item) => ({
                    ...item,
                    expanded: false,
                }));
    ...
    const handleClearFieldText = (status) => {
        ...
        const hasActiveFilters = isAdavnceSearch || searchCommunicationName;

        if (!status && hasActiveFilters) {
            const currentPageSize = payload?.pagination?.pageSize || 5;
            setMyCommunicationsOnly(true);
            myCommunicationsOnlyRef.current = true;
    ...
    const hanldeSearchClearField = async (status) => {
        ...
        setNameSuggestions([]);
        setNameSuggestLoading(false);
        if (!status && (searchTextRef.current || payload.searchCommunicationName)) {
            const currentPageSize = payload?.pagination?.pageSize || 5;
            searchTextRef.current = '';
            setSearchValueSync((prev) => ({ rev: prev.rev + 1, value: '' }));
            const scopeMy = syncAnalyticsMyCommunicationsScopeFromStorage();
