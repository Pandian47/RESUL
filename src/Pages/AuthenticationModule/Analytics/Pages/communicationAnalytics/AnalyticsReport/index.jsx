...
import { getSummaryList, getRetargetListStatus } from 'Reducers/analytics/analyticsSummary/selector';

import { useDispatch, useSelector } from 'react-redux';
import { getCommunicationSummary, getGoldenCampaign, getGeography, GetRetargetListStatus, getSnapNameList, getSnapDetails } from 'Reducers/analytics/analyticsSummary/request';
import HeaderIcons from './Components/HeaderIcons';
import RoiChart from './Components/Overview/Components/RoiChart';
import { getSessionId } from 'Reducers/globalState/selector';
import { resetCSRState, updateSummarySubSegmentDetail } from 'Reducers/analytics/analyticsSummary/reducer';
...
import {
    ...
    consumeAnalyticsReportRouteSkeleton,
...
import { preparePdfExport } from './utils/pdfIframeSnapshot';
...
const AnalyticsReport = () => {
    ...
    const audienceDetailsModalRef = useRef({ open: null, close: null });
    ...
    const [showRouteSkeletonShell, setShowRouteSkeletonShell] = useState(false);
    const shouldShowPageSkeleton = summaryLoading || showRouteSkeletonShell;
    ...
    useLayoutEffect(() => {
        if (consumeAnalyticsReportRouteSkeleton()) {
            setShowRouteSkeletonShell(true);
        }
    }, []);

    useEffect(() => {
        if (!summaryLoading) {
            setShowRouteSkeletonShell(false);
        }
    }, [summaryLoading]);
    //PDF
    ...
    const exportPDFWithComponent = () => {
        ...
        try {
            ...
            const errorHandler = (message, source, lineno, colno, error) => {
                if (isPDFError(message, error?.toString())) {
            ...
            const networkErrorHandler = (event) => {
                ...
                if (isPDFError(event.message, event.error?.toString(), resourceUrl)) {
    ...
    const { summarySubSegmentDetail, retargetListLoading, snapshotList } = useSelector(({ analyticsReportReducer }) => analyticsReportReducer);
    const { subSegmentDetailInDetailPage } = useSelector(({ analyticsDetails }) => analyticsDetails);
    ...
    const handleSummaryPageAPI = async (subSegmentLevel, isSubSegment) => {
        ...
        if (payload && state?.from) {
            ...
            const {status} = await dispatch(getCommunicationSummary({ payload, loading: false }));
            if (!status) {
    ...
    useEffect(() => {
        ...
        if (state?.snapshotId && state?.snapshotName) {
            ...
            dispatch(getSnapDetails({ payload: snapPayload, loading: false }));
        } else {
            handleSummaryPageAPI();
        }
            ...
            const factModals = _map(sortedChannels, (list, index) => ({
                ...
                component: () => {
                    return retargetListLoading ? (
                        <div className="arsk-channel-analytics-body-slot" aria-hidden="true">
                            <AnalyticsReportChannelAnalyticsBodySkeleton />
                        </div>
                    ) : (
                    ...
                    <div
                        className={`page-content-holder${
                            shouldShowPageSkeleton
                                ? ' analytics-report-skeleton-scope analytics-report-page-loading'
                                : ''
                        }${isDownloadUI ? ' download-page-setup download-page-setup-detail' : ''}`}
                        style={shouldShowPageSkeleton ? ANALYTICS_REPORT_LOADING_HOLDER_STYLE : undefined}
                        aria-busy={shouldShowPageSkeleton ? 'true' : undefined}
                    >
                        ...
                        {shouldShowPageSkeleton ? (
                            ...
                            <Overview
                                ...
                                date={getUserCurrentFormat(summary?.jobDateTime,{isOffset:true})?.utcformat}
                            />
                            ...
                            {analyticsTabAlert && (
                                <RSModal
                                    ...
                                    footer={
                                        <RSPrimaryButton
                                            onClick={() => {
                                                navigate(state?.fromPath || `/analytics`);
                                            }}
                                        >
                                            {OK}
                                        </RSPrimaryButton>
