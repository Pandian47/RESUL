...
const PushNotifySetting = () => {
    ...
    const watchPushNotifySetting = watch('digipop.pushNotifySetting');

    const getUniqueFielArrayName = (name = '', index) => {
        const uniqueName = `digipop.pushNotifySetting.pushNotifyResolution[${index}]`;
        return `${uniqueName}.${name}`;
    };
                ...
                return {
                    ...
                    component: () => (
                        <RenderField
                            ...
                            item={item}
                            key={item?.id ?? index}
                            fieldName={'digipop.pushNotifySetting.pushNotifyResolution'}
                        />
                    ),
            ...
            <Row>
                <Col sm={{ offset: 1, span: 8 }} className="position-relative">
                    ...
                    <RSTabbar
                        ...
                        onRemoveMenu={(index) => {
                            remove(index);
                            setValue(getUniqueName('pushNotifyDefaultTab'), index - 1);
                        }}
                        customIconsize="xs"
                    />
                </Col>
