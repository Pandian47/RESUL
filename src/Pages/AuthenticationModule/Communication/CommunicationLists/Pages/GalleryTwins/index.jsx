import { CHANNELSSOCIAL_LIST } from 'Utils/modules/communicationChannels';
...
const GALLERY_EXCLUDE_ACTIVE_FILTER_CHIP_KEYS = [...COMMUNICATION_ADVANCE_SEARCH_EXCLUDE_NAME_CHIP_KEYS];
...
const CommunicationGallery = () => {
    ...
    const galleryChannelLabel = useMemo(() => {
        const hit = CHANNELSSOCIAL_LIST.find((c) => {
            if (c.id !== payload.channelId) return false;
            if (payload.channelId === 7 || payload.channelId === 10) {
                return Number(c.subChannelId) === Number(payload.subChannelId);
            }
            return true;
        });
        ...
        async (text, armGen) => {
            ...
            try {
                const response = await dispatch(
                    getSearchDropdownDataCommunicationList({ payload: searchPayload, loading: false }),
                );
                if (armGen !== nameSuggestGenRef.current) return;
    ...
    useEffect(() => {
        ...
        if (!isAgencyAccountAdmin) {
            ...
            dispatch(getGalleryList({ payload: tempParams }));
        }
    }, [payload]);
    ...
    const refreshGalleryList = useCallback(() => {
        ...
        lastGalleryListRequestRef.current = null;
        dispatch(getGalleryList({ payload: tempParams }));
    }, [payload, departmentId, departmentName, licenseTypeId, isAgencyAccountAdmin, dispatch]);
    ...
    const handleInfoGallery = async (infoData, campaignId, channelId, subChannelId = 0) => {
        ...
        try {
            const { data, status } = await dispatch(getInfoList({ payload: infoPayload, loading: false }));
            if (status) {
                    ...
                    <div className="flex-row justify-content-end top-sub-heading advanceSearchContainer">
                         <RSAdvanceSearchNew
                            ...
                            auxiliaryRightControls={
                                <div className={isLoading ? 'pe-none opacity-60' : ''}>
                                    <BootstrapDropdown
                                        data={GALLERY_SCOPE_DROPDOWN_DATA}
                                        defaultItem={
                                        ...
                                        onSelect={(item) => {
                                            ...
                                            handleMyCommunicationsToggleRef.current(next);
                                        }}
                                    />
                                </div>
                            }
                                    ...
                                    <i
                                        ...
                                        onClick={() => {
                                            if (addAccess) {
                                                ...
                                                navigateTab({ field: 'commPlanTab', data: null });
