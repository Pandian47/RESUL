...
import { mapAudienceWithChannelLabels, numberWithCommas } from 'Utils/modules/formatters';
...
const Messaging = ({ type = 'whatsapp', mCampType, channelId }) => {
    ...
    const { savedChannelsId, channelAudiences = {} } =
    ...
    const { messaging: messagingTabState } = tabsState;
    const { type: channelType, currentTab } = verticalTab;
    ...
    const { tabSmartLink_Flag, customFields, mobileApps, screenList, subScreenList } =
        useSelector(({ smartLinkReducer = {} }) => smartLinkReducer) || {};
    ...
    const calucateAudienceCount = useMemo(() => sumAudienceCountByField(audience, 'recipientCountMobile'), [audience]);
    ...
    useEffect(() => {
        contactLoader
            ...
            .then((rslt) => {
                ...
                const { status, data: { phoneNo, countryCode } = {} } = rslt;
                ...
                try {
                    ...
                    if (status && hasCampaignDetails(data)) {
                        ...
                        [ensureArray(audienceData ?? audienceList), ensureArray(filterAudienceResponse?.data)].forEach(
                            (item) => {
                                const updateAudienceData = _map(ensureArray(item), (audience) => ({
                                    ...mapAudienceWithChannelLabels(audience),
    ...
    const scrollToHeaderError = (errs) => {
        const hasHeaderTypeError =
            errs?.headerType || Object.keys(errs || {}).some((key) => key.startsWith('split') && errs[key]?.headerType);
        ...
        try {
            ...
            if (
                ...
                if (status) {
                    ...
                    dispatch(
                        updateChannelAudiences(mergeChannelAudiences('Whatsapp', selectedAudience, channelAudiences)),
                    );
                }
                ...
                } else if (formType === 'test preview' || formType === 'live') {
                    if (status && campaign.campaignType === 'M') {
                        handleUpdateChannelDetailId(data);
                    }
                    formTypeRef.current = getTestType();
                    const isLivePreview = formType === 'live';
                    if (status) {
                        // Clear dirty state after successful test/live preview save while keeping form values
                        reset(getValues(), { keepValues: true });
                        dispatch(updateDirtyState(false));

                        UpdateState(
                            setState,
                            ['isScheduled', 'sentCommunicationModal', 'testSentCommunicationModal', 'isLivePreview'],
                            [false, true, '', isLivePreview],
                        );
                    } else {
                        UpdateState(
                            setState,
                            ['isScheduled', 'sentCommunicationModal', 'testSentCommunicationModal', 'isLivePreview'],
                            [false, true, message, isLivePreview],
                        );
                    }
                    setTimeout(() => {
                        UpdateState(
                            setState,
                            ['testSentCommunicationModal', 'sentCommunicationModal', 'isLivePreview'],
                            [false, false, false],
                        );
                        setSmsPreview(false);
                    }, 5000);
    ...
    useEffect(() => {
        if (
            ...
            checkRFAApproved(getCampaignStatusId(campaignDetails), campaignDetails?.requestForApproval?.approvarList)
        ) {
            setIsClickOff(true);
        } else {
            setIsClickOff(false);
    ...
    function handleTemplate(templateDetails = {}, isSplitAB = false, fieldName = '', isEdit = false, content) {
        ...
        if (isCarousel) {
            ...
            formData.previewImage = isLocationType
                ? ''
                            ...
                            <div className="form-group mt20">
                                <Row>
                                    <Col sm={{ offset: 1, span: 2 }}>
                                        <label className="control-label-left">{SENDER} name</label>
                                    </Col>
                        ...
                        <div className="buttons-holder">
                            ...
                            {disableNext && (
                                <>
                                    ...
                                    >
                                        {mCampType === 'M' ? `${mdcButtonText} WhatsApp content` : NEXT}
                                    </RSPrimaryButton>
                                </>
                            )}
                        </div>
                    ...
                    {state.sentCommunicationModal && (
                        <CommunicationSent
                            ...
                            handleClose={() => {
                                ...
                                UpdateState(
                                    setState,
                                    ['sentCommunicationModal', 'rfaMsg', 'requestFalse', 'isLivePreview'],
                                    [false, false, '', false],
                                );
                            }}
                            smsPreview={state.isLivePreview}
                        />
                    )}
