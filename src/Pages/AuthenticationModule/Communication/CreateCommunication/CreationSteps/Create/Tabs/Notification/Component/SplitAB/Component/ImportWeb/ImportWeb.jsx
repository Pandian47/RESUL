...
import WebPreviewConfig from '../WebPreviewConfig/WebPreviewConfig';
import RSWebPreview from 'Pages/AuthenticationModule/Communication/Component/RSWebPreview';

import { getImportCampaign, uploadCommunicationFileWeb, uploadMessagingVideoDocument, uploadWebPush } from 'Reducers/communication/createCommunication/Create/request';
...
const ImportWeb = ({
  ...
  const inPageBannerName = 'inPageBanner';
  ...
  const importUrlLabelText = 'Import URL';
  ...
  const checkBannerDimensions = useCallback(() => {
    ...
    // Get image / video dimensions if they exist
    let imageWidth = 0;
    let imageHeight = 0;
  ...
  const setIframeHeight = (iframe, body) => {
    ...
    try {
      ...
      if (edmelement && contentHeight > 0) {
        edmelement.style.height = `${scaledHeight}px`;
        edmelement.style.maxHeight = `${scaledHeight}px`;
        edmelement.style.overflow = 'hidden';
      }
      ...
      if (content) {
        ...
        if (isSimpleImageTag) {
          ...
          if (imageSrc) {
            ...
            // Add wrapper to template (host may be sibling of empty-state <small> in RSWebPreview)
            if (templateElement) {
              templateElement.innerHTML = '';
        ...
        // Simple <video>…</video> (banner media URL / file)
        ...
        if (isSimpleVideoTag) {
          ...
          if (videoSrc) {
            ...
            videoElement.addEventListener(
              'loadedmetadata',
              () => {
                if (templateElement && videoElement.videoWidth > templateElement.offsetWidth && templateElement.offsetWidth > 0) {
  ...
  const getVideoMimeFromUrl = (url) => {
    ...
    const map = { mp4: 'video/mp4', webm: 'video/webm', ogg: 'video/ogg', mov: 'video/quicktime', m4v: 'video/x-m4v' };
  ...
  const processImportUrl = useCallback(
    async (rawUrl, opts = {}) => {
      ...
      // In-page banner: allow image, GIF (image), and video URLs
      ...
      if (isImageUrl) {
        ...
        if (!isValidExtension) {
          const msg =
          'Image URL only supports .png, .jpeg, .jpg, .gif extensions only';
          setError(importURLName, {
            type: 'custom',
            message: msg
          });
        ...
        if (extension === 'mp4') {
            ...
            result = await zipUploadLoader.refetch({
                fetcher: ({ payload: mediaPayloadInner } = {}) =>
                    dispatch(uploadWebPush({ payload: mediaPayloadInner, loading: false })),
                mode: 'create',
                loaderConfig: AUTHORING_FIELD_LOADER_CONFIG,
  ...
  const handleImportFileUpload = async (file) => {
    ...
    reader.onload = async () => {
      ...
      if (isNotification && (!isInPageBannerDeliveryType || isZipExtension(extension))) {
  ...
  <Fragment>
          ...
          className={`${isClickOff ? 'pe-none click-off' : ''} position-relative`}>
            ...
            isEmoji={false} />
                ...
                <div
      className={`d-flex ${viewInBrowser ? 'justify-content-end align-items-center' : 'justify-content-end'} my15`
      }>
    ...
    <>
        ...
        'import-url-selected'}`
              ...
              onClick={() => setValue(importTypeName, 'import')}>
                                ...
                                <div className="rs-import-with-icon or-sep">
                                    <div className="import-zip-file-tab text-center position-relative top4">
                                        ...
                                        <label className="rsiwi-label control-label-left">
                                            {importFileLabelText}
                                        </label>
                                        <Fragment>
                                            <input
                      ...
                      {...register(zipFileName, {
                        ...
                        validate: async (value) => {
                          const edmContentVal = await getValues(edmContentName);
                          ...
                          return isInPageBannerDeliveryType ?
                          'Select media file' :
                          'Select zip file';else
                          {
                            if (edmFileWeight > MAX_IMPORT_FILE_SIZE)
                            return 'File size too large';
                            return true;
                                        ...
                                        <div className="import-zip-message position-relative">
                                            {zipUploadLoader.isLoading && (
                                                <span
                                                    ...
                                                    id="rs_ImportWeb_zip_upload_loader"
                                                >
          ...
          <Fragment>
                                                    ...
                                                    <div className="import-zip-message rs-import-inpage-zip-message position-relative">
                                                        {zipUploadLoader.isLoading && (
                                                            <span
                                                                className="d-inline-flex align-items-center justify-content-center position-absolute"
                                                                style={{ right: 20, top: 27 }}
                                                                id="rs_ImportWeb_inpage_zip_upload_loader"
                                                            >
                                                                <span className="segment_loader"></span>
                                                            </span>
                                                        )}
                    ...
                    title={
                    isSplit ? getValues(`${fieldName}.title`) : getValues('title')
                    }
                    buttonText={
                    isSplit ?
                    getValues(`${fieldName}.buttonText`) :
                    getValues('buttonText')
                    } />
