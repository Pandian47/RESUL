...
import { COMMUNICATION_PERFORMANCE, TOP_LINK_ACTIVITY, USER_ENGAGEMENT, changeToBase64, defaultValues, getPreviewData, handleChannelInfo, handleSegmentData, handleSplit, mapChartOption, pieChartOption } from '../../constants';
...
import { getDetailReport_OverviewDetails, getkycount } from 'Reducers/analytics/details/request';
...
const DetailAnalyticsQr = ({ type, isDownloadUI }) => {
    ...
    const typeLowercase = type.toLowerCase().split(' ').join('');
    const channelId = getChannelId(typeLowercase)?.id;
    const { overviewDetail, channelDetail, segmentDetail, fromSplitHeader, isLoading, updatekyccount, segmentId } = useSelector(({ analyticsDetails }) => analyticsDetails);
    ...
    // const { channelDetail } = useSelector(({ analyticsDetails }) => analyticsDetails);
    ...
    useEffect(() => {
        if (keyMetrics && locationData?.campaignId) {
            const kycPayload = {
                campaignId: locationData?.campaignId,
                formId: channelDetail?.formId || 0
            };
            dispatch(getkycount({payload: kycPayload}));
        }
    }, [keyMetrics, locationData?.campaignId, channelDetail?.formId]);
    return (
        ...
        {(Object.keys(overviewDetail)?.length > 0 ) ? 
            <>
                ...
                <Row>
                            ...
                            <AreasPlineChart
                                areaPlineDataList={
                                    blastAudienceJson && hasNonZeroEngagementData(blastAudienceJson)
                                        ? changeToBase64(blastAudienceJson)
                                        : {}
                                }
                            />
                        </Row>
                    ...
                    <Col md={8} className="position-relative">
                        <TabbarAreasPlineChart
                            heading={'Reach'}
                            splitItemText={
                                splitItem === 'A' ? `Above counts are based on day-wise unique audience` : ''
                            }
                    ...
                    <Col md={4}>
                        <KeyMetricsNew
                            data={keyMetrics !== undefined && keyMetrics !== null ? keyMetrixData({...keyMetrics, kyccount: updatekyccount}) : []}
                ...
                <Row>
                    <Col md={6} className="x-axis-labels-performance">
                        <TabbarAreasPlineChart
                          ...
                          overAllPercent={Number(reach?.uniqueScans ?? 0).toFixed(1)}
                        overAllText={
                            <>
                                total reach with &nbsp;
                                <strong class='font-bold'>{engagement?.lnkclicks}%</strong>&nbsp;
                                engagement rate occurred during this period
                            </>
                            }
                            first24HoursPercent={reach?.uniqueScans}
                            first24HoursText={<>
                                total reach with &nbsp;
                                <strong class='font-bold'>{engagement?.lnkclicks}%</strong>&nbsp;
                                engagement rate occurred during this period
                ...
                <Row>
                    ...
                    <Col md={6}>
                        <MapChart
                            title={'By location'}
                            chartData={
                                locationInfoJson !== '' && locationInfoJson !== undefined
                                    ? mapChartOption(locationInfoJson)
                                    : []
                            }
                            footerPercent={formatNumber(engagement?.lnkclicks)}
                            footerText={`of users from ${topLocation} have highest engagement rate`}
                        />
                    </Col>
                    <Col md={6}>
                        <PieChart
                            title={'Scans by operating systems'}
                            chartData={
                                !!popularityInfoJson && popularityInfoJson?.length !== 0 && hasValidPieChartData(popularityInfoJson)
                                    ? pieChartOption(popularityInfoJson, 'columnFooter',true)
                                    : {}
                            }
                            footerPercent={formatNumber(topOsValue)}
                            footerText={`of qr code have been scanned from ${topOs} operating systems `}
                        />
                    </Col>
                </Row>
                <Row>
                ...
                <ClickMapModal
                    ...
                    scheduleDate={date}
                    channelId={locationData?.channelId ?? channelId}
                />
...
export default DetailAnalyticsQr;
