import { Audience360Top5Skeleton } from 'Components/Skeleton/Skeleton';
...
const Top5Channels = () => {
    ...
    useEffect(() => {
        let channelTopData = [];
        if (channels === 'Reach') {
            channelTopData = JSON.parse(overView?.reachTopChannel || '[]');
        } else if (channels === 'Engagement') {
            channelTopData = JSON.parse(overView?.engTopChannel || '[]');
        } else if (channels === 'Conversion') {
            channelTopData = JSON.parse(overView?.convTopChannel || '[]');
        }
    ...
    return (
        <div className="portlet-container portlet-md channelList">
            ...
            <div className="portlet-body">
                <Row className="m0">
                    {!overView?.loading && !!channelsData && channelsData?.length !== 0 ? (
                        ...
                        <Col>
                            <Audience360Top5Skeleton
                                nodata={overView?.loading && !!channelsData && channelsData?.length !== 0 ? false : true}
                            />
                        </Col>
                    )}
                </Row>
