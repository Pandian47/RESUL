...
import { ACCESS_POINT, AUTHORIZATION_KEY, CANCEL, PASSWORD, PROVIDER, SENDER_DETAILS, SENDER_ID, SENDER_NAME, TEMPLATE, TEMPLATE_NAME, USER_NAME, WHATSAPP_RESPONSES, WHATSAPP_SETTINGS } from 'Constants/GlobalConstant/Placeholders';
...
const WhatsAppCreate = ({ type, config, handleCancel, setFailedApi }) => {
    ...
    useEffect(() => {
        dispatch(getCSWACreateProviders({ type: 'W', departmentId, clientId, userId }));
        if (isUpdate) getUpdateData();
    }, []);

    const getUpdateData = async () => {
        const res = await dispatch(
            getCSWAUpdateGet({ clientWASettingID: config?.clientWASettingId, departmentId, clientId, userId }),
        );
        const { status, data } = res;
        if (status) {
            let { accessPoint, authorizationKey, waResponse, waSender, serviceProviderId, userName, password } = data;
            ...
            reset(
                (prev) => ({
                    ...
                    userName,
                    password,
                    whatsAppResponse: waResponse?.map((res) => ({
                        ...res,
                        isDelete: false,
                    })),
                    senderDetails: waSender?.map((res) => ({
                        ...res,
                        isDelete: false,
                    })),
    ...
    useOnlyDepChangeEffect(() => {
        if (isUpdate) setValue('provider', _find(providers, ['serviceProviderId', providerId]));
    }, [providers]);
    ...
    const handleFormSubmit = (formState) => {
                // handleCancel(true);
        let waSender = formState.senderDetails?.map((res) => ({
            clientwaSenderID: isUpdate ? res.clientwaSenderId : 0,
            senderId: res.senderId,
            senderName: res.senderName,
            isDelete: isUpdate ? res?.isDelete : false,
        }));
        let waResponse = formState.whatsAppResponse?.map((res) => ({
            clientwaResponseID: isUpdate ? res.clientWAResponseId : 0,
        ...
        const payload = {
            ...
            clientId,
            userId,
            clientwaSettingId: isUpdate ? config.clientWASettingId : 0,
            // "clientID": clientId,
            ...
            waSender: [...waSender, ...senderAndResponse?.sender],
            waResponse: [...waResponse, ...senderAndResponse?.response],
        };
        dispatch(createCSWASettings(payload, handleCancel));
    };
        ...
        <FormProvider {...methods}>
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                        ...
                        <Row>
                            ...
                            <Col sm={6}>
                                <RSInput
                                    type={'text'}
                                    name={'accessPoint'}
                                    id="rs_WhatsAppCreate_accesspoint"
                        ...
                        <Row>
                            <Col sm={6}>
                                <RSInput
                                    ...
                                    id="rs_WhatsAppCreate_authorizationKey"
                                            ...
                                            <Row>
                                                <Col sm={11} className={'pr0'}>
                                                    <RSInput
                                                        ...
                                                        id="rs_WhatsAppCreate_senderName"
                ...
                <div className="buttons-holder">
                    <RSSecondaryButton type="button" onClick={() => handleCancel(true)} id="rs_WhatsAppCreate_Cancel">
                       {CANCEL}
                    </RSSecondaryButton>
                    {addAccess && (
                        <RSPrimaryButton type="submit" className={``} id="rs_WhatsAppCreate_Update">
