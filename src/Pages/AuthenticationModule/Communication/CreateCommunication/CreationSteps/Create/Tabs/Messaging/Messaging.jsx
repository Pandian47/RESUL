...
} from 'Utils/modules/campaignUtils';
...
import {
    ...
    AUDIENCE,
    AUDIENCE_CHANGE_CONFIRMATION,
    AUDIENCE_COUNT_ZERO_ENABLE_AUTO_REFRESH,
    AUTO_REFRESH,
    AUTO_REFRESH_POP_HOVER_TEXT,
    AUTO_SCHEDULE_SPLITS,
    CANCEL,
    CHECK_START_DATE_AND_END_DATE,
...
import {
    ...
    editActionIdFromCommunicationResponse,
...
const Messaging = ({ type = 'SMS', mCampType, channelId }) => {
    ...
    let [splitTest, audience, isRequestApproval, senderName, senderId, phoneNumber, err, dailCode, watchtotalAudience] =
        watch([
            'splitTest',
            'audience',
    ...
    const calucateAudienceCount = useMemo(() => sumMobileAudienceCount(audience), [audience]);
    ...
    const getUsersList = async () => {
        try {
            ...
            const { status, data } = usersRes || {};
            let userList = ensureArray(status ? data : []);
    ...
    useEffect(() => {
        contactLoader
            .refetch({
                fetcher: ({ payload } = {}) => dispatch(getContactByUserId({ payload, loading: false })),
            ...
            .then((rslt) => {
                if (!isMounted.current) return;
                if (!rslt) return;
                const { status, data } = rslt || {};
                const { phoneNo, countryCode } = data || {};
    ...
    }, [location?.campaignId, campaignDetails]);
    ...
    useEffect(() => {
        async function fetchEditFlow() {
            ...
            const mdcChannelDetailId = _get(location?.mdcContentSetupDetails, 'channelDetailId', 0);
            const payload = {
                userId,
                clientId,
                departmentId,
            ...
            if (
                ...
                try {
                    ...
                    if (!isMounted.current) return;
                    const [senderRes, audienceRes, campaignRes] = editApiResult || [];
                    const { data: smsSenderList } = senderRes || {};
                    const { data: audienceData } = audienceRes || {};
                    const { status, data } = campaignRes || {};
                    if (status && !!Object.keys(ensureObject(data))?.length) {
                        ...
                        if (
                            !audience?.length &&
                            filterAudience?.length &&
                            ensureSegmentationListIds(segmentationListId)?.length
                        ) {
                        ...
                        let templateList = [];
                        {
                            ...
                            if (!isMounted.current) return;
                            const findTemplate = _find(
                                templateList,
                                (item) => String(item.dltTemplateID) === String(data?.content?.[0]?.templateId),
                            );
                            ...
                            ensureMessagingContent(content).forEach(
                                ({ body, localBlastDateTime, splitType, timeZoneId, templateId }) => {
                                    ...
                                    tempTab.templateId =
                                        _find(
                                            templateList,
                                            (item) => String(item.dltTemplateID) === String(templateId),
                                        ) || templateId;
                                    tempTab.isCustomTemplateId = _find(
                                        templateList,
                                        (item) => String(item.dltTemplateID) === String(templateId),
                                    )
                                        ? false
                        ...
                        if (sanitizeMessagingCount(smsSplit?.splitAudience, 0) > 0) {
                            ...
                            tempSplitAudience.tabs = _map(
                                ensureMessagingContent(content),
                        ...
                        const matchAudienceType = audienceTypeList?.filter((typeList) =>
                            ensureArray(audience)
                                ?.map((aud) => aud?.listType)
                                ?.includes(typeList?.id),
                        );
            ...
            if (
                ...
                if (formState.splitTest) {
                    ...
                    for (let i = 0; i < tempSplitTabsList?.length; i++) {
                        const currentTab = formState?.[tempSplitTabsList[i]];
                        const selectedSender = getValues('senderId');
                        const isTemplateIdRequired = !!(selectedSender?.clientSmsSettingId || selectedSender > 0);
                        const resolvedTemplateId =
                            typeof currentTab?.templateId === 'object' && currentTab?.templateId !== null
                                ? (currentTab?.templateId?.dltTemplateID ?? currentTab?.templateId?.templateId)
                                : currentTab?.templateId;

                        if (isTemplateIdRequired && !String(resolvedTemplateId ?? '').trim()) {
                            errorIndex = i;
                            errorField = 'templateId';
                            break;
                        }
                    ...
                    if (
                        ...
                        if (nullCount === tempSplitTabsList?.length) {
                            ...
                            for (var k = 0; k < scheduleAll?.length; k++) {
                                ...
                                let checkAllSplitNoSchedule = Object.values(scheduleAll)?.every(
                                    (schedule) => !schedule,
                                );
                    ...
                    if (errorIndex !== undefined && errorField) {
                        ...
                        const isCustomTemplateId = getValues(`${tempSplitTabsList[errorIndex]}.isCustomTemplateId`);
                        const errorMessages = {
                            editorText: ENTER_EDITOR_TEXT,
                            templateId: isCustomTemplateId
                                ? ENTER_TEMPLATE_ID
                        ...
                        setTimeout(() => {
                            setValue('currentSplitTab', errorIndex);
                ...
                const saveRes = await runSave(getAuthoringSaveButtonType(formType), () =>
                    dispatch(saveMessagingCampaign({ payload, type, savedChannelsId, channelId, loading: false })),
                );
                if (!isMounted.current) return;
                const { status, data, message = EXCEPTION_OCCURRED } = saveRes || {};
    ...
    const fetchSMSTemplateList = async (senderId) => {
        try {
            ...
            if (status) {
                ...
                finalResponse = ensureArray(data).map((templateData) => ({
                    ...templateData,
                    subLabel: templateData?.dltTemplateID,
                }));
    ...
    return (
        <MessagingContext.Provider value={memoizedContextValue}>
                        ...
                        <div className="buttons-holder">
                            ...
                            {disableNext && (
                                <>
                                    <RSSecondaryButton
                                        ...
                                        onClick={() => {
                                            ...
                                            if (
                                                isCTGTConfirm &&
                                                !hasUserConfirmed &&
                                                !handleCGTGModalCheck(campaignDetails?.content?.[0]?.statusId)
                                            ) {
                                    ...
                                    >
                                        {mCampType === 'M' ? `${mdcButtonText} SMS content` : NEXT}
                                    </RSPrimaryButton>
                                </>
                            )}
                        </div>
