import { USER_ENGAGEMENT } from '../../communicationAnalytics/DetailedAnalytics/constants';
...
const FormAnalyticsContent = ({ formAnalytics }) => {
    const locationData = useQueryParams('/preferences/template-gallery/form-analytics');
    const { progProfileData, progProfileLoading } = useSelector(({ analyticsDetails }) => analyticsDetails);
    const {
        reach = {},
        engagement = {},
        conversion = {},
    ...
    return (
        <>
            <div className="rs-csr-wrapper">
                ...
                {lastUpdatedtime && (
                    <small className="font-normal text-muted ml10">(As on: {getUserCurrentFormat(lastUpdatedtime, { isOffset: true })?.dateTimeFormat})</small>
                )}
                {/* Overview Cards */}
                <Row>
                            ...
                            <ColumnChart
                                ...
                                footerText={
                                    <>
                                        ...
                                        <small className="font-bold pr5">%</small> of conversion happened and{' '}
                ...
                <Row>
                    ...
                    <Col md={6}>
                        {/* <PieChart
                            title={'Channels'}
                            chartData={pieChartData.device.chartData}
                            footerPercent={pieChartData.device.hasData ? pieChartData.device.topValue.value : undefined}
                            footerText={
                                pieChartData.device.hasData
                                    ? `of the link clicks were by ${pieChartData.device.topValue.name || 'channels'}`
                                    : ''
                            }
                        /> */}
                        <div className="box-design">
                            ...
                            <div className="portlet-header flex-row mb0">
                                ...
                                <div
                                    className={`d-flex align-items-center ${
                                        progressiveData?.length > 0 ? '' : 'pe-none click-off'
                                    }`}
                                >
                                    <RSTooltip
                                        text={'Download CSV'}
