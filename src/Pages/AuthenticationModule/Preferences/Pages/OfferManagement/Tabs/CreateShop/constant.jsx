...
// Build payload for Shop save API
export const buildShopPayload = (data, clientId, departmentId, userId, storeId = null) => {
    const {
        brand,
        shortName,
        legalName,
    ...
    const payload = {
        ...
        departmentId,
        userId,
        StoreLattitude : latitude,
        StoreLongitude : longitude,
        brandId: brand?.brandID || brand || null,
        storeShortName: shortName || null,
        storeLegalName: legalName || null,
        storeImage: image || null,
        storeDescription: description || null,
        storeContactName: contactName || null,
        storeEmailID: emailId || null,
        storeMobileNo: mobileNo || null,
        storeCategoryID: categoryId || null,
        storeSubCategoryID: subCategoryId || [],
        storeCurrency: currency?.currencyID || currency || null,
        storeAddress: address || null,
        storeCity: city || null,
        storeCountry: country?.countryID || country || null,
        storeStatus: status?.id !== undefined 
            ? (status.id === 1 ? true : false) 
            : (status !== undefined ? (status === 1 ? true : false) : null),
    };

    // Add storeId to payload if in edit mode
    if (storeId) {
...
// Map API response to Shop form state (similar to getSateObject in CreateOffer)
export const getShopStateObject = ({ shopData, brandList, currencyMasterList, countryMasterList, brandId, categoryList, subCategoryList }) => {
    if (!shopData) return {};

    // Handle array response structure - get first item if array
    const data = Array.isArray(shopData) ? shopData[0] : shopData;
    if (!data) return {};
    ...
    const stateObject = {
        ...
        status: data.status !== undefined 
            ...
            : STATUS_OPTIONS[0],
             latitude : data?.storeLattitude,
            longitude : data?.storeLongitude
    };

    // Set brand if brandId is provided and brandList is available
    // API response has brandID field, use that if available, otherwise use brandId parameter
    const brandIdToUse = data.brandID || brandId;
    if (brandIdToUse && brandList && brandList.length > 0) {
        const selectedBrand = brandList.find(b => b.brandID === brandIdToUse);
        if (selectedBrand) {
            stateObject.brand = selectedBrand;
        }
    }
