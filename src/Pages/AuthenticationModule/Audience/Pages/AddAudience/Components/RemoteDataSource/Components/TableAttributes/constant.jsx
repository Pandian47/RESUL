...
export const generateLeftAttributes = (left, right) => {
    // this method filter left attr values from right attr
    let table = left[0]?.table;
    let rightAttr = right?.filter((e) => e?.table === table);
    let leftAttr = left?.filter((e) => !rightAttr?.some((ee) => ee.name === e.name) && e);
    return leftAttr;
};
...
// export const finalPayload_CRM = (mySql, getValues, type) => {
//     const { ipAddress, userName, password, databaseName, portNumber, instanceName, api, updatedCycle } = getValues();
//     const tableName = _map(mySql, (res, key) => {
//         return key;
//     });
//     let columnName = Object.keys(mySql).map((ele, ind) => {
//         let keys = mySql[ele];
//         return keys.attributes.rightAttributes.map((e) => e.name && e.name).join();
//     });
...
//         scheduleFrequency: updatedCycle.typeId,
//         columns:
//             type?.data.remoteDataSourceID === 45 || type?.data.remoteDataSourceID === 28 ? columnName : columnName[0],
//         tableName:
//             type?.data.remoteDataSourceID === 45 || type?.data.remoteDataSourceID === 28
//                 ? tableName.toString()
//                 : tableName[0],
//     };
// };
...
export const Component = ({ dataItem, ...others }, table) => {
    return (
        <li {...others} key={others.id} className={`${others.className} d-block`}>
            ...
            {dataItem?.name}
            {/* // ICON_UPDATE {Stephen}: Update the icons for below */}
...
export const attributeColumn = (RDSID, isDateFieldAvailable = false) => {
    return  [
        ...
        ...(INTEGRATED_SQL_CONNECTOR_IDS.includes(RDSID) || isDateFieldAvailable ? [{
            field: 'dateType',
            title: 'Date field',
        }] : []),
        {
