...
const ConsumptionRcs = () => {
    ...
    const buildGridData = useCallback(() => {
        if (!consumptionChannelList?.rcsChannelConsumptionList) return [];

        const allItems = consumptionChannelList?.rcsChannelConsumptionList;
        ...
        return gridRows;
    }, [consumptionChannelList?.rcsChannelConsumptionList, processCustomFields]);
    ...
    useEffect(() => {
        const tableName =
            Array.isArray(consumptionChannelList?.rcsChannelConsumptionList?.[0]?.custom) &&
            consumptionChannelList.rcsChannelConsumptionList[0].custom?.length > 0
                ? _map(consumptionChannelList.rcsChannelConsumptionList[0].custom, (res, key) => {
        ...
        <Fragment>
            ...
            {consumptionChannelList?.length > 0 || consumptionChannelList ? (
                <>
                    ...
                    />
                    {consumptionChannelList?.rcsChannelConsumptionList?.length !== null ? (
                        <div className="mb70">
                            <KendoGrid
                                data={gridData}
                                isLoading={isLoading}
