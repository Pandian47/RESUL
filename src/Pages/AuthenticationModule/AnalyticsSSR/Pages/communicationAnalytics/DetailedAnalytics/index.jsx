...
import { getWarningPopupMessage, getSseDisconnectPopup } from 'Utils/modules/warningPopup';
...
import { DETAIL_ANALYTICS_TAB_CONFIG, handleDefaultTab, SEGMENT_ENABLED_CAMPAIGNS } from './constants';
import { resolveDetailAnalyticsBackPath } from 'Pages/AuthenticationModule/Analytics/Pages/communicationAnalytics/DetailedAnalytics/constants';
...
import {
    get_Digipop_Reports,
    getDetailReport_ChannelDetails,
    getDetailReport_OverviewDetails,
    getDetailReport_SegmentDetails,
    closeDetailReportSSE,
    retryDetailReportOverview,
    setDetailReportSseRetryHandler,
} from 'Reducers/analyticsSSR/details/request';
...
} from 'Reducers/analyticsSSR/details/reducer';
...
const DetailAnalytics = () => {
    ...
    const locationData = useQueryParams('/AnalyticsSSE/detail-analytics');
    ...
    useEffect(() => {
        return () => {
            closeDetailReportSSE();
            setDetailReportSseRetryHandler(null);
        };
    }, []);
    ...
    // const { detailsList } = useSelector(({ analyticsDetailsSSR }) => analyticsDetailsSSR);
    const { channelDetail, isLoading, isFailure, isChannelLoading, sseDisconnected, ...otherAnalyticsDetails } = useSelector(({ analyticsDetailsSSR }) => analyticsDetailsSSR);
    ...
    const getCampDetails = async (segmentLevel, isSubsegmentLevelChange, channelId) => {
        if (!isDigipopCamp) {
            ...
            if (status) {
                const currentChannelId = locationData?.channelId;
                if (data?.channelInfos && Array.isArray(data.channelInfos) && (currentChannelId === 7 || currentChannelId === 10)) {
                    const filteredChannelInfos = data.channelInfos.filter(item => item.channelId === currentChannelId);
                    
                    dispatch(updateDetailsMainList({ 
                        field: 'channelDetail', 
                        data: { ...data, channelInfos: filteredChannelInfos }
                    }));
                }
                
                const channelInfos = data?.isSplitEnabled && data?.splitChannelLevelInfo?.length ? data?.splitChannelLevelInfo : (data?.channelInfos && data?.channelInfos?.length) ? data.channelInfos : [];
                let splitIndex = null
                ...
                const preferredIndex = isSubsegmentLevelChange
                    ? 0
                    : analyticsDetatils?.currIndex ?? locationData?.currIndex ?? 0;
                const boundedIndex =
                    preferredIndex >= 0 && preferredIndex < channelInfos.length ? preferredIndex : 0;
                const selectedChannelInfo = channelInfos[splitIndex || boundedIndex] || channelInfos[0] || {};
            ...
            const encryptState = encodeUrl(pathState);
               if (locationData?.channelId === 7 || locationData?.isFromListingPage) {
                setTempBackPath(`/AnalyticsSSE`)
                //tempBackPath = `/analytics`;
            } else {
                setTempBackPath(`/AnalyticsSSE/analytics-report?q=${encryptState}`)
    ...
    const getCampDetailsRef = useRef(getCampDetails);
    getCampDetailsRef.current = getCampDetails;

    useEffect(() => {
        setDetailReportSseRetryHandler(async () => {
            dispatch(setSseDisconnected(false));
            closeDetailReportSSE();
            lastDetailFetchKeyRef.current = null;
            isAlreadyCalledAPi.current.status = false;
            dispatch(resetAnalyticsDetailState());
            await getCampDetailsRef.current();
        });
        return () => setDetailReportSseRetryHandler(null);
    }, [dispatch]);
    ...
    useEffect(() => {
        if ((channelDetail !== '' && Object.keys(channelDetail)?.length && locationData !== null) || isDigipopCamp) {
            ...
            tabs =
                locationData?.channelId === 7 || locationData?.channelId === 10
                    ? [
                          {
                              id: 0,
                              text: locationData?.channelId === 7 ? 'Social media' : 'Paid media',
                              ...
                              component: DETAIL_ANALYTICS_TAB_CONFIG(isDownloadUI).filter((detail) => {
                                  // console.log('CHANNEL ID::', detail.channelId);
                                  let tempchannelId = locationData?.channelId;

                                  return detail.channelId === tempchannelId;
                              })?.[0]?.component,
                          },
                      ]
                    : processChannelInfos(sortedChannels)?.map((item, inde) => {
                          const tabItem = {
                              id: inde,
                              text:
                                  channelDetail?.campaignType === 'M'
                                      ...
                                      : item?.channelId === 10
                                      ? 'Paid media' 
                                      : item?.channelFriendlyName || getChannelId(item?.channelId)?.label, // item?.channelFriendlyName,
                              disable: false,
                              blastShortCode: item?.blastShortCode,
                              channelId: item?.channelId ?? locationData?.channelId,
                              campaignId: locationData?.campaignId,
                              href: getTabUrl({
                                  channelId: item?.channelId ?? locationData?.channelId,
                                  blastShortCode: item?.blastShortCode,
                              ...
                              component: DETAIL_ANALYTICS_TAB_CONFIG(isDownloadUI).filter((detail) => {
                                  // console.log('CHANNEL ID::', detail.channelId);
                                  let tempchannelId =
                                      locationData?.channelId === 7
                                          ? 7
                                          : locationData?.channelId === 10
                                          ? 10
                                          : item?.channelId;
            ...
            if (analyticsDetatils?.channelId === 10) {
                defaultIndex = 0;
            } else if (isAlreadyCalledAPi.current.isSubsegmentLevelChange) {
                defaultIndex = 0;
    ...
    const handleSelectedTab = async (selectedItem) => {
        const payload = {
            ...
            userId,
            campaignId: locationData?.campaignId, // analyticsDetatils?.campaignId,
            blastID: selectedItem?.blastShortCode || locationData?.blastId,
            channelId: selectedItem?.channelId,
        };
        let blastId = selectedItem?.blastShortCode || locationData?.blastId;
        let channelId = selectedItem?.channelId;
        let currIndex = selectedItem?.id;
        let detailList = { ...analyticsDetatils, blastId, channelId, currIndex };
    ...
    useEffect(() => {
        if (tabListRef.current?.tabsListView?.length) {
            ...
            let targetIndex = 0;
            if (analyticsDetatils?.currIndex !== undefined && analyticsDetatils?.currIndex >= 0) {
                targetIndex = analyticsDetatils?.currIndex;
            } else if (locationData?.currIndex !== undefined && locationData?.currIndex >= 0) {
    ...
    return (
        <DetailAnalyticsProvider.Provider value={contextValue}>
            ...
            >
                ...
                >
                    <RSPageHeader
                        ...
                        isBack
                        // backPath={`/analytics`}
                        backPath={resolveDetailAnalyticsBackPath({
                            locationData,
                            tempBackPath,
                            defaultCsrPath: '/AnalyticsSSE/analytics-report',
                        })}     
                        isBackAsLink
                        rightCommonMenus
                                        ...
                                        renderItem={
                                            channelDetail?.csrSubSegmentList?.length ? (
                                                <RSBootstrapdown
                                                    ...
                                                    showUpdate
                                                    defaultItem={
                                                        Object.keys(otherAnalyticsDetails?.subSegmentDetailInDetailPage || {}).length > 0 ? otherAnalyticsDetails?.subSegmentDetailInDetailPage : channelDetail?.csrSubSegmentList[0]
                                                    }
                                                    className="float-end"
            ...
            {getSseDisconnectPopup({
                show: sseDisconnected,
                onRetry: () => dispatch(retryDetailReportOverview()),
                onClose: () => dispatch(setSseDisconnected(false)),
            })}
        </FormProvider>
